      ...
      <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all ${scrolled ? 'border-slate-200/80 bg-white/90 shadow-sm backdrop-blur-xl' : 'border-white/10 bg-[#0A1328]/90 backdrop-blur-md'}`}>
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className={`hidden items-center gap-7 text-sm font-semibold lg:flex ${scrolled ? 'text-slate-600' : 'text-slate-200'}`} aria-label="التنقل الرئيسي">
             ...
             <a href={route('/guide')} className="transition hover:text-primary">دليل الاستخدام</a>
         ...
         {menuOpen && <div className="border-t border-slate-100 bg-white px-4 py-4 sm:hidden"><nav className="grid gap-1">{[['المميزات', '/features'], ['دليل الاستخدام', '/guide'], ['الحلول المتكاملة', '/solutions'], ['لماذا ترصيد', '/why-tarseed'], ['الأمان والأداء', '/security-performance'], ['الخطط', '/pricing']].map(([label, href]) => <a key={href} href={route(href)} className="rounded-lg px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">{label}</a>)}<button type="button" onClick={() => openAuth('login')} className="mt-2 rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm font-bold text-slate-800 hover:bg-slate-50" data-testid="button-mobile-login">تسجيل الدخول</button><button type="button" onClick={() => openAuth('register')} className="rounded-lg bg-primary px-3 py-3 text-sm font-bold text-white" data-testid="button-mobile-register">ابدأ تجربتك المجانية</button></nav></div>}
      </header>
        ...
        <section className="relative overflow-hidden bg-[#0A1328] px-4 pb-20 pt-36 text-white sm:px-6 lg:pb-28 lg:pt-48">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1fr_0.9fr]">
            <div className="max-w-2xl">
              ...
              <h1 className="text-5xl font-black leading-[1.12] tracking-tight sm:text-6xl lg:text-7xl">إدارة أذكى.<br /><span className="text-teal-300">نمو أسرع.</span></h1>

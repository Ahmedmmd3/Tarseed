          ...
          <div className="space-y-8">
            ...
            <CrudTable
              ...
              fields={[
                ...
                { key: 'rating', label: 'تقييم المورد', type: 'select', options: [{ value: '5', label: '⭐⭐⭐⭐⭐ ممتاز' }, { value: '4', label: '⭐⭐⭐⭐ جيد جداً' }, { value: '3', label: '⭐⭐⭐ جيد' }, { value: '2', label: '⭐⭐ مقبول' }, { value: '1', label: '⭐ ضعيف' }] },
                { key: 'status', label: 'حالة المورد', type: 'select', options: [{ value: 'active', label: 'نشط' }, { value: 'suspended', label: 'موقف مؤقتاً' }, { value: 'blacklisted', label: 'محظور' }], defaultValue: 'active', renderCell: (value) => { const status = String(value || 'active'); const label = status === 'active' ? 'نشط' : status === 'suspended' ? 'موقف مؤقتاً' : 'محظور'; const tone = status === 'active' ? 'bg-emerald-100 text-emerald-700' : status === 'suspended' ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700'; return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${tone}`}>{label}</span>; } },

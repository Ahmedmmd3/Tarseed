...
export default function WorkspacePrep({ title }: { title: string }) {
  return (
    <section className="overflow-hidden rounded-[28px] border border-white/10 bg-white shadow-2xl shadow-slate-950/15" data-testid="page-workspace-prep">
      <div className="bg-gradient-to-l from-teal-500 to-[#0a426f] px-6 py-8 text-white sm:px-8">
      ...
      <div className="px-6 py-8 sm:px-8"><h2 className="text-xl font-black text-slate-900">الوحدة موجودة في لوحة التحكم</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">أصبحت هذه الوحدة ظاهرة في القائمة الرئيسية ولوحة التشغيل حتى يبقى الوصول إلى كل عمليات منشأتك واضحاً. سنربط إجراءاتها وسجلاتها التشغيلية هنا دون أن تؤثر على حساباتك أو صلاحياتك الحالية.</p><Link href="/dashboard" className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-[#062b4d] px-5 text-sm font-bold text-white transition hover:bg-[#0a426f]" data-testid="button-back-to-dashboard">العودة إلى لوحة التحكم <ArrowLeft className="h-4 w-4" /></Link></div>
    </section>
  );
}
...
function TransferWorkspace({ transfers, products, warehouses, loading, onChanged }: {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <LoadingState /> : <div className="rounded-2xl border border-slate-100 bg-white shadow-sm"><Table><TableHeader><TableRow><TableHead>المنتج</TableHead><TableHead>من</TableHead><TableHead>إلى</TableHead><TableHead>الكمية</TableHead><TableHead>الحالة</TableHead><TableHead>الإجراء</TableHead></TableRow></TableHeader><TableBody>
  ...
  return (
    <div className="flex flex-col gap-6" data-testid="page-inventory">
      ...
      <Tabs defaultValue="products" className="w-full">
        <TabsList className="mb-6 flex h-auto flex-wrap gap-2 md:grid md:w-full md:grid-cols-5"><TabsTrigger value="products">المنتجات</TabsTrigger><TabsTrigger value="warehouses">المواقع</TabsTrigger><TabsTrigger value="balances">الأرصدة</TabsTrigger><TabsTrigger value="transfers">التحويلات</TabsTrigger><TabsTrigger value="adjustments">التسويات</TabsTrigger></TabsList>
        <TabsContent value="products"><CrudTable table="products" title="إدارة المنتجات" fields={[{ key: 'name', label: 'الاسم', required: true }, { key: 'sku', label: 'رمز المنتج' }, { key: 'price', label: 'سعر البيع', type: 'number' }, { key: 'cost', label: 'سعر التكلفة', type: 'number' }]} /></TabsContent>
